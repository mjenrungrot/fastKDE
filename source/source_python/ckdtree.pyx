...
cdef class cKDTree:
    ...
    cdef int __query_max_distance(cKDTree self,
            np.float64_t*result_distances,
            np.intp_t*result_indices,
            np.float64_t*x,
            np.intp_t k,
            np.float64_t eps,
            np.float64_t p,
            np.float64_t distance_lower_bound) except -1:
        ...
        cdef np.float64_t d
        cdef np.float64_t epsfac
        cdef np.float64_t max_distance
        cdef np.float64_t far_min_distance
        cdef heapitem it, it2, neighbor
        cdef leafnode* node
        cdef innernode* inode
        ...
        # priority queue for the furthest neighbors
        # closest known neighbor first
        # entries are (distance**p, i)
        neighbors = heap(k)
        ...
        try:
            ...
            # internally we represent all distances as distance**p
            if p!=infinity and distance_lower_bound!=0:
                distance_lower_bound = distance_lower_bound**p
            ...
            while True:
                if inf.node.split_dim==-1:
                    ...
                    for i in range(node.start_idx,node.end_idx):
                        d = _distance_p(
                                self.raw_data+self.raw_indices[i]*self.m,
                                x,p,self.m,distance_lower_bound)

                        if d>distance_lower_bound:
                            # replace closest neighbor
                            if neighbors.n==k:
                                neighbors.remove()
                            neighbor.priority = d
                            neighbor.contents.intdata = self.raw_indices[i]
                            ...
                            # adjust upper bound for efficiency
                            if neighbors.n==k:
                                distance_lower_bound = neighbors.peek().priority
                    ...
                    # but since the distance_upper_bound decreases, we might get
                    # here even if the cell's too far
                    if max_distance < distance_lower_bound*epsfac:
                    ...
                    if p == infinity:
                        # we never use side_distances in the l_infinity case
                        # inf2.side_distances[inode.split_dim] = dabs(inode.split-x[inode.split_dim])
                        far_max_distance = dmin(max_distance, dabs(inode.split-x[inode.split_dim]))
                    elif p == 1:
                        inf2.side_distances[inode.split_dim] = dabs(inode.split-x[inode.split_dim])
                        far_max_distance = max_distance - \
                            inf.side_distances[inode.split_dim] + \
                            inf2.side_distances[inode.split_dim]
                    else:
                        inf2.side_distances[inode.split_dim] = dabs(inode.split -
                    ...
                    it2.priority = far_max_distance


                    # far child might be too far, if so, don't bother pushing it
                    if far_max_distance>=distance_lower_bound*epsfac:
                        q.push(it2)
                    else:
                        stdlib.free(inf2)
                        inf2 = <nodeinfo*> NULL
    ...
    def query(cKDTree self, object x, np.intp_t k=1, np.float64_t eps=0,
        ...
        # furthest
        dd2 = np.empty((n,k),dtype=np.float64)
        dd2.fill(0)
        ii2 = np.empty((n,k),dtype=np.intp)
        ii2.fill(self.n)

        for c in range(n):
            self.__query(&dd[c, 0], &ii[c, 0], &xx[c, 0],
                         k, eps, p, distance_upper_bound)
            #self.__query_max_distance(&dd2[c, 0], &ii2[c, 0], &xx[c, 0],
            #                          k, eps, p, 0)
            ...
            if k==1:
                if sizeof(long) < sizeof(np.intp_t):
                    ...
                    if ii[0,0] <= <np.intp_t>LONG_MAX:
                        return dd[0,0], int(ii[0,0]), dd2[0, 0], int(ii2[0, 0])
                    else:
                        return dd[0,0], ii[0,0], dd2[0,0], ii2[0,0]
                else:
                    # ... most other platforms
                    return dd[0,0], ii[0,0], dd2[0,0], ii2[0,0]
            else:
                return dd[0], ii[0], dd2[0], ii2[0]
        else:
            if sizeof(long) < sizeof(np.intp_t):
                # ... e.g. Windows 64
                for i in range(n):
